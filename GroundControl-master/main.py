...
class GroundControlApp(App):
    ...
    def computeSettings(self, section, key, value):
        ...
        if key == 'kinematicsType':
            ...
            sprocketRadius = (float(self.config.get('Advanced Settings', 'chainPitch')) / 2.0 / math.sin (3.141592 / float(self.config.get('Advanced Settings', 'gearTeeth'))) + distPerRot/2.0/3.141592)/2.0
            self.config.set('Computed Settings', "distPerRot", str(distPerRot))
            self.config.set('Computed Settings', "sprocketRadius", str(sprocketRadius))
