...
void  Kinematics::triangularInverse(float xTarget,float yTarget, float* aChainLength, float* bChainLength){
    ...
    */
    ...
    float leftMotorDistance = sqrt(pow((sysSettings.leftMotorX - xTarget),2)+pow((sysSettings.leftMotorY - yTarget),2));
    float rightMotorDistance = sqrt(pow((sysSettings.rightMotorX - xTarget),2)+pow((sysSettings.rightMotorY - yTarget),2));
    ...
    float leftChainStraight = sqrt(pow(leftMotorDistance,2)-pow(sysSettings.sprocketRadius,2));
    float rightChainStraight = sqrt(pow(rightMotorDistance,2)-pow(sysSettings.sprocketRadius,2));

    //Correct the straight chain lengths to account for chain sag
    leftChainStraight *= (1 + ((sysSettings.chainSagCorrection / 1000000000000) * pow(cos(leftChainAngle),2) * pow(leftChainStraight,2) * pow((tan(rightChainAngle) * cos(leftChainAngle)) + sin(leftChainAngle),2)));
    ...
    rightChain = rightChain - sysSettings.rotationDiskRadius;

    *aChainLength = leftChain;
    *bChainLength = rightChain;
}
